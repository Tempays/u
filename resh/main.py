...
def show_city(user_id):
    ...
    user = get(f'http://localhost:{PORT}/api/users/{user_id}').json()
    if not user:
        return make_response(jsonify({'error': 'Not found'}), 404)
    geocode = user.json['city_from']
    ...
    return render_template('users_show.html', name=user.json["name"], surname=user.json["surname"],
                           city_from=user.json["city_from"], user_id=user_id)
